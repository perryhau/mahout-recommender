...
public class TestPoissonHypothesis extends TasteTestCase {
  Hypothesis hypothesis;
  Vector[] x, theta;
  @Before
  public void setUp(){
    hypothesis = new PoissonHypothesis();
  }
  @Test
  public void predict(){
    ...
    assertArrayEquals(new double[]{Math.exp(10)},hypothesis.predict(x, theta),0.001);
    ...
    x = new Vector[]{x4};
    theta = new Vector[]{t4};
    assertArrayEquals(new double[]{Math.exp(4)}, hypothesis.predict(x, theta), 0.001);
    assertArrayEquals(hypothesis.predict(x, theta), hypothesis.predict(new double[]{4}), 0.001);

    x4.setQuick(4, 0);
    assertArrayEquals(new double[]{1}, hypothesis.predict(new Vector[]{x4}, new Vector[]{t4}), 0.001);
